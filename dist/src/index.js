            ...
            try {
                ...
                switch (name) {
                    ...
                    case "list_customer_reviews": {
                        const { appId, filterTerritory, filterRating, existsPublishedResponse, sort, fieldsCustomerReviews, fieldsCustomerReviewResponses, limit, include } = request.params.arguments;
                        if (!appId) {
                            throw new McpError(ErrorCode.InvalidParams, "appId is required");
                        }
                        const params = {};
                        if (filterTerritory?.length)
                            params["filter[territory]"] = filterTerritory.join(",");
                        if (filterRating?.length)
                            params["filter[rating]"] = filterRating.join(",");
                        if (typeof existsPublishedResponse === "boolean")
                            params["exists[publishedResponse]"] = existsPublishedResponse;
                        if (sort?.length)
                            params["sort"] = sort.join(",");
                        if (fieldsCustomerReviews?.length)
                            params["fields[customerReviews]"] = fieldsCustomerReviews.join(",");
                        if (fieldsCustomerReviewResponses?.length)
                            params["fields[customerReviewResponses]"] = fieldsCustomerReviewResponses.join(",");
                        if (typeof limit === "number")
                            params["limit"] = Math.min(Number(limit), 200);
                        if (include?.length)
                            params["include"] = include.join(",");
                    ...
                    case "list_customer_reviews_for_version": {
                        const { versionId, filterTerritory, filterRating, existsPublishedResponse, sort, fieldsCustomerReviews, fieldsCustomerReviewResponses, limit, include } = request.params.arguments;
                        if (!versionId) {
                            throw new McpError(ErrorCode.InvalidParams, "versionId is required");
                        }
                        const params = {};
                        if (filterTerritory?.length)
                            params["filter[territory]"] = filterTerritory.join(",");
                    ...
                    case "create_customer_review_response": {
                        const { reviewId, responseBody } = request.params.arguments;
                        if (!reviewId || !responseBody) {
                            throw new McpError(ErrorCode.InvalidParams, "reviewId and responseBody are required");
                        }
                        const body = {
                            data: {
                    ...
                    case "delete_customer_review_response": {
                        const { responseId } = request.params.arguments;
                        if (!responseId) {
                            throw new McpError(ErrorCode.InvalidParams, "responseId is required");
                        }
                    ...
                    case "get_customer_review_response": {
                        ...
                        if (!reviewId) {
                            throw new McpError(ErrorCode.InvalidParams, "reviewId is required");
                        }
